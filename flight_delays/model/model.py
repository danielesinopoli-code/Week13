...
class Model:
    def __init__(self):
        ...
        self._nodi = []
        self._archi = []
        # COSTRUISCO LISTA E DIZIONARIO DI  AEROPORTI
    ...
    def buildGrafo(self,min):
        #NODI
        self._nodi= DAO.getNodes(min, self._dizionarioAeroporti)
        self._grafo.add_nodes_from(self._nodi)
        #ARCHI
        self._archi= DAO.getEdges(self._dizionarioAeroporti)
        for c in self._archi:
    ...
    def getSortedNeighbors(self, v0):
        vicini = self._grafo.neighbors(v0) #Trova tutti i vicini
        viciniTuple = []
        for v in vicini: # Creo tuple(vicino, peso)
            viciniTuple.append((v, self._grafo[v0][v]["weight"]))
            viciniTuple.sort(key=lambda x: x[1], reverse=True) #ordino per peso
        return viciniTuple
